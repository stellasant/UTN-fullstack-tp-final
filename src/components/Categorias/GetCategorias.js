...
export default function GetCategorias() {
    ...
    return (
        <div>
            ...
            {errordelete && errordelete.map(err => <div key={1}><p>{err}</p></div>)}
            {listado && listado.map(unaCategoria => (
                <div key={unaCategoria.id} className='categoria'>
                    <h3>{unaCategoria.nombre}<button onClick={() => handleDelete(unaCategoria.id)}>Eliminar</button></h3>
                </div>
            ))}
